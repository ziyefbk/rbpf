        ...
        if let Some(baseline) = baseline_result {
            ...
            for result in &test_case.results {
                ...
                if correct {
                    ...
                    if Tnum::new(baseline.output.value, baseline.output.mask).contains(Tnum::new(result.output.value, result.output.mask)) {
                        s.less_than += 1;
                    } else if Tnum::new(result.output.value, result.output.mask).contains(Tnum::new(baseline.output.value, baseline.output.mask)) {
                        s.more_than += 1;
                    } else {
                        s.not_equal += 1;
                    }
